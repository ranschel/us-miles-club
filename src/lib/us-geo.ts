// State FIPS <-> USPS code + name mapping. Source: US Census Bureau.
// Used across the map, dropdowns, and leaderboards.
...
export function stateFipsFromCode(code: string): string | undefined {
  return STATE_BY_CODE[code]?.fips;
}

export function stateCodeFromFips(fips: string): string | undefined {
  return STATE_BY_FIPS[fips]?.code;
}
